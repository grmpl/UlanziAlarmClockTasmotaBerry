"""
This program is for setting BUZZER-GPIO back to normal mode after it was set to RTC-mode for ULP-control
Additionally it will stop ULP from running and clear registers, so it's an extended cleanup
var c = bytes().fromb64("dWxwAAwAPAAIAAAAAAHsHSgBzBkGAGAcBgDAL/EAgHIEAABoKAGwLQEBgHIEAABoAACAcgEAgHICAIByAwCAcgAAQHQAAACwAAAAAAAAAAA=")
ULP.load(c)
ULP.run()
...
.global registercntl
registercntl: .long 0
registertouch: .long 0
.text 
.global entry
entry:
WRITE_RTC_REG(RTC_GPIO_OUT_REG, RTC_GPIO_OUT_DATA_S + BUZZERGPIO, 1, 0)
WRITE_RTC_REG(RTC_IO_TOUCH_PAD3_REG, RTC_IO_TOUCH_PAD3_MUX_SEL_M, 1, 0);
